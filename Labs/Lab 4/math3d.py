...
class Ray(object):
    def __init__(self,origin,destination):
        ...
        self.destinationNorm=destination.normalized()
        if isinstance(self.origin,VectorN) and isinstance(self.destination,VectorN) and len(self.origin)==(len(self.destination)):
            pass
        else:
            return ValueError("One of the Vectors isnt a VectorN or isnt of equal length.")
    def getPt(self,t):
        N=self.origin+((t)*self.destinationNorm)
        return N
    def drawPygame(self,screen,color,width):
        destPoint=self.getPt(1000)
        pygame.draw.line(screen,color,(self.origin[0],self.origin[1]),(destPoint[0],destPoint[1]),width)
    def getDistanceToPoint(self,Pos):
        ...
        VecPara=((Vector.dot(self.destinationNorm))/self.destinationNorm.dot(self.destinationNorm)*self.destinationNorm)
        VecPerp=Vector-VecPara
        if self.destinationNorm.dot(Vector)<0:
            return None
        #VecMag=VecPara.cross(VecPerp)
        #Vec=Vec.dot(self.AngleFacing)
        return VecPerp.magnitude()
        #come back to work on a none statment.

#This is my Camera Class
class Camera(object):
    def __init__(self,CameraPos,cameraObjInterest,up,near,fieldOfView,screen):
